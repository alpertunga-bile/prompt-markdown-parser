...
class GenerateTab:
    ...
    def __init__(self, parent, tab):
        ...
        self.generatorEntry = CTkEntry(
            master=self.variableFrame, placeholder_text="E.g. gpt2"
        )
        self.generatorPathLabel = CTkLabel(
            master=self.variableFrame, text=self.generatorPath
        )

        chooseGenerator = CTkButton(
            master=self.variableFrame, text="Choose Model", command=self.ChooseGenerator
        )

        minLengthLabel = CTkLabel(
            master=self.variableFrame, text="Minimum number of generated tokens"
        )
        self.minLengthEntry = CTkEntry(master=self.variableFrame, placeholder_text="10")

        maxLengthLabel = CTkLabel(
            master=self.variableFrame, text="Maximum number of generated tokens"
        )
        ...
        self.seedEntry = CTkEntry(
            master=self.thisTab,
            placeholder_text="Enter Your Seed e.g. goddess",
            width=400,
        )

        self.generatorTextbox = CTkTextbox(master=self.thisTab, width=500, height=200)

        self.recursiveLabel = CTkLabel(
            master=self.variableFrame, text="Recursive Level : 0"
        )
        ...
        self.earlyStoppingCheckbox.grid(
            column=1,
            row=4,
        )
        ...
        generateButton = CTkButton(
            master=self.thisTab, text="Generate Text", command=self.StartGenerate
        )
    ...
    def Generate(self):
        ...
        minLength = (
            10 if self.minLengthEntry.get() == "" else int(self.minLengthEntry.get())
        )
        maxLength = (
            50 if self.maxLengthEntry.get() == "" else int(self.maxLengthEntry.get())
        )
        ...
        happy_gen = HappyGeneration(
            upperModelName, modelName, load_path=self.generatorPath
        )
    ...
    def SliderEvent(self, value):
        self.recursiveLabel.configure(
            text=f"Recursive Level : {int(self.recursiveSlider.get())}"
        )
