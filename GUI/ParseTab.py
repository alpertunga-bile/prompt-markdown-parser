...
class ParseTab:
    ...
    def __init__(self, parent, tab):
        ...
        translatorCombobox = CTkComboBox(
            ...
            state="readonly",
            values=["Google", "Mymemory", "Linguee", "PONS"],
        )
        ...
        initializeButton = CTkButton(
            master=self.initializeFrame, text="Parse And Save", command=self.Parse
        )

        informationLabel = CTkLabel(master=self.initializeFrame, text="")

        progressBar = CTkProgressBar(
            master=self.initializeFrame, orientation="horizontal", mode="determinate"
        )
        progressBar.set(0)
