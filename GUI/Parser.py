...
class Parser:
    ...
    def SelectDirectory(self) -> None:
        ...
        text = (
            "1 file is selected"
            if len(self.promptFiles) == 1
            else f"{len(self.promptFiles)} files are selected"
        )
    ...
    def SelectTranslator(self) -> None:
        ...
        for promptFile in self.promptFiles:
            if exists(promptFile) == False:
                self.textLabel.configure(
                    text=f"{promptFile} is not exists | Skipping ..."
                )
                continue
    ...
    def ParseAndSave(self, promptFile: str) -> None:
