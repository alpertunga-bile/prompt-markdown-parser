...
class DatasetTab:
    ...
    def __init__(self, parent, tab):
        ...
        self.datasetLinkLabel = CTkLabel(
            master=self.variableFrame, text=self.datasetPath
        )
        ...
        positiveNameLabel = CTkLabel(
            master=self.variableFrame, text="Dataset Positive Filename"
        )
        self.positiveFileEntry = CTkEntry(
            master=self.variableFrame, placeholder_text="E.g. positive"
        )
        negativeNameLabel = CTkLabel(
            master=self.variableFrame, text="Dataset Negative Filename"
        )
        self.negativeFileEntry = CTkEntry(
            master=self.variableFrame, placeholder_text="E.g. negative"
        )
        ...
        createDatasetButton = CTkButton(
            master=self.thisTab, text="Create Dataset", command=self.CreateDataset
        )
        createDatasetButton.pack()

        self.progressBar = CTkProgressBar(
            master=self.thisTab, orientation="horizontal", mode="determinate"
        )
